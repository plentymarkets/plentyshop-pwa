import { type BasketItemOrderParamsProperty, DoAddItemParams, Order, orderGetters } from '@plentymarkets/shop-api';
import {
  type AddOrderToCart,
  ...
  const addOrderToCart: AddOrderToCart = async () => {
    ...
    state.value.order?.order.orderItems.forEach((orderItem) => {
      ...
      if (
        ...
        orderGetters.hasAllOrderPropertiesReady(orderItem)
      ) {
        items.push({
