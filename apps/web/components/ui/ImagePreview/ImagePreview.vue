<template>
  <div class="relative w-full border rounded overflow-hidden min-h-[500px] flex flex-col bg-editor-mint">
    <div class="flex justify-between items-center p-4 ">
      <div class="text-sm text-gray-700">
        <strong>{{ props.name }}</strong>
        <span v-if="props.dimensions" class="ml-2 text-gray-500">({{ props.dimensions }})</span>
      </div>
      <button v-if="props.name && props.image" class="text-gray-500 hover:text-gray-700 text-xl leading-none" @click="close">
        &times;
      </button>
    </div>
