  ...
  <div class="relative flex first:border-t border-b-[1px] border-neutral-200 hover:shadow-lg min-w-[320px] p-4 last:mb-0"
    ...
    <div class="relative overflow-hidden rounded-md w-[100px] sm:w-[176px]">
      <SfLink :tag="NuxtLink"
        ...
        <NuxtImg class="w-full h-auto border rounded-md border-neutral-200" :src="cartItemImage || '/images/product.webp'"
          :alt="cartItemImage || ''" width="300" height="300" loading="lazy" format="webp" />
      </SfLink>
    </div>
    <div class="flex flex-col pl-4 min-w-[180px] flex-1">
      <SfLink :tag="NuxtLink"
        :to="`${paths.product}${cartGetters.getItemName(cartItem)}-${cartGetters.getVariationId(cartItem)}`"
        variant="secondary" class="no-underline typography-text-sm sm:typography-text-lg">
      ...
      <div class="items-start sm:items-center sm:mt-auto flex flex-col sm:flex-row">
        <span v-if="prices.special"
          ...
          <BasePrice :basePrice="productGetters.getDefaultBasePrice(cartItem.variation)"
            :unitContent="productGetters.getUnitContent(cartItem.variation)"
            :unitName="productGetters.getUnitName(cartItem.variation)" :content-line-first="true" :oneline="false" />
        </span>
        ...
        <UiQuantitySelector @change-quantity="debounceQuantity" :value="cartGetters.getItemQty(cartItem)" :min-value="1"
          class="mt-4 sm:mt-0" />
      </div>
    </div>
