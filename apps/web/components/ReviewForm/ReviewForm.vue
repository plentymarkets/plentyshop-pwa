    ...
    >
      <div class="col-span-2">
        <div class="flex items-center justify-between">
          <p :id="ratingLabelId" class="typography-label-sm font-medium text-neutral-900">{{ $t('review.yourRating') }}</p>
          <SfRatingButton v-model="ratingModelValue" :aria-labelledby="ratingLabelId" class="p-1 gap-x-2" />
        </div>
        <label class="my-4 block">
          <span class="block typography-label-sm font-medium mb-0.5 text-neutral-900">{{ $t('review.reviewText') }}</span>
          <textarea
              v-model="reviewModelValue"
              placeholder="Describe your experience eg. Great product! The qual a reliable and durable solution."
        ...
        <label class="block mb-6">
          <span class="block mb-0.5 typography-label-sm font-medium text-neutral-900">{{ $t('review.reviewAuthor') }}</span>
          <SfInput v-model="usernameModelValue" />
        </label>
        <div class="flex justify-end gap-x-4">
          <SfButton type="button" variant="secondary" class="flex-1 md:flex-initial" @click="$emit('on-close')">{{ $t('review.cancel') }}</SfButton>
          <SfButton type="submit" class="flex-1 md:flex-initial" @click="$emit('on-submit')">{{ $t('review.submitReview') }}</SfButton>
        </div>
      </div>
    </form>
  </div>
