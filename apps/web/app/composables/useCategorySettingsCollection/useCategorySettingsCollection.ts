...
export const useCategorySettingsCollection: useCategorySettingsCollectionReturn = () => {
  ...
  const addCategorySettings = async (category: CategoryEntry) => {
    const categoryDetail = category.details?.[0];
    if (!categoryDetail) return;
    
    const exists = state.value.data.some(
      (item) => {
        const itemDetail = item.details?.[0];
        return item.id === category.id && itemDetail?.lang === categoryDetail.lang;
      },
    );
    if (exists) return;
      ...
      const settings = JSON.parse(
        JSON.stringify(
          dirtyCategories
            ...
            .map((category) => {
              const detail = category.details[0]!; // Safe since we filtered above
              return {
                id: category.id,
