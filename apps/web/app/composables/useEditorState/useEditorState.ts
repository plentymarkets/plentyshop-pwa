 ...
 * Centralizes all `$isEditor` logic into semantic, well-named computed properties
...
export const useEditorState = (): UseEditorStateReturn => {
  ...
  const isPreviewMode = computed(() => !!$isEditor && !disableActions.value);

  /** @description Whether we are in live/production mode (no editor features) */
  const isLiveMode = computed(() => !$isEditor);
