...
export const useBlockManager = () => {
  ...
  const getTemplateByLanguage = (category: string, variationIndex: number, lang: string) => {
    ...
    if (!variationsInCategory) {
      throw new Error(`Category ${category} not found in blocks lists`);
    }
    const variationToAdd = variationsInCategory.variations[variationIndex];
    if (!variationToAdd) {
      throw new Error(`Variation at index ${variationIndex} not found in category ${category}`);
    }
