<template>
  <div v-if="graduatedList.length > 0" class="typography-text-sm font-bold mt-5">
    {{ t('product.graduatedPrices.title') }}
  </div>
  <table v-if="graduatedList.length > 0" class="w-full text-left typography-text-sm mb-2">
    <thead class="border-b-2 border-neutral-200">
      <tr>
        <th class="lg:py-4 py-2 lg:pr-4 pr-2 font-medium">{{ t('product.graduatedPrices.price') }}</th>
        <th class="lg:p-4 p-2 font-medium lg:whitespace-nowrap">{{ t('product.graduatedPrices.quantity') }}</th>
        <th class="lg:p-4 p-2 font-medium">{{ t('product.graduatedPrices.discount') }}</th>
      </tr>
    </thead>
    <tbody>
      <tr
