<template>
  ...
  >
    ...
    <div :data-testid="`slide-settings-${activeSlide}`">
      <BlocksBannerCarouselBannerForm v-if="activeSlide !== undefined && slides[activeSlide]" :uuid="slides[activeSlide]!.meta.uuid" />
      <UiAccordionItem
...
const moveSlideUp = async (index: number) => {
  ...
  // Ensure both elements exist before swapping
  if (newSlides[index - 1] && newSlides[index]) {
...
const moveSlideDown = async (index: number) => {
  ...
  // Ensure both elements exist before swapping
  if (newSlides[index] && newSlides[index + 1]) {
