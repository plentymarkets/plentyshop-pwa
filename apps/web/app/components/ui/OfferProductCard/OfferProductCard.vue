  ...
  <div v-for="(variation, variationId, key) in props.offer.variations" :key="variationId">
    <div class="relative flex border-neutral-200 border-b min-w-[320px] p-4 last:mb-0">
      ...
      <div class="pl-2 w-2/3">
        ...
        <p v-if="filteredOffer.order.orderItems[key]">{{ format(offerGetters.getOfferItemOriginalPrice(filteredOffer.order.orderItems[key]!)) }}</p>
        ...
        <p class="flex justify-end self-end text-yellow-600 font-bold text-lg pt-6">
          <span v-if="filteredOffer.order.orderItems[key]">{{ format(offerGetters.getOfferItemOriginalPriceWithQuantity(filteredOffer.order.orderItems[key]!)) }}</span>
        </p>
      </div>
    </div>
  </div>
