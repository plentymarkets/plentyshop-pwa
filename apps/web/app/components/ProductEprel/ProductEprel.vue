<template>
  <div data-testid="producteprel" class="flex items-center gap-2 absolute" style="position: absolute;right: 0; top: 0; z-index: 2;">
    <ArrowSvg
      v-if="showArrow"
      :level="eprelValue"
      :size="size"
...
const eprelProperty = computed(() =>
  productPropertyGetters.getProperty(EPREL_PROPERTY_ID, propertyGroups.value || [])
);
...
const eprelValue = computed(() =>
  eprelProperty.value
    ? productPropertyGetters.getPropertyValue(eprelProperty.value)
    : ''
);
