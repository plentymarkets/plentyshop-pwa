<template>
  <div v-if="!modalOpen">
    <div class="flex flex-wrap items-center gap-1.5 p-2 bg-gray-50 border-b border-gray-200" data-testid="rte-toolbar">
      ...
      <EditorColorPicker data-testid="rte-font-color" :model-value="textColor" dropdown-align="rte" @update:model-value="setFontColor($event)">
      ...
      <button v-if="expandable" type="button" class="w-8 h-8 rounded border border-transparent bg-transparent inline-flex items-center justify-center text-sm cursor-pointer hover:bg-gray-100 ml-auto" :aria-expanded="expandedLocal" data-testid="rte-expand" @mousedown.prevent @click="expandedLocal = !expandedLocal">
    ...
    <div v-if="expandable && expandedLocal" class="flex flex-wrap items-center gap-1.5 p-2 bg-gray-50 border-b border-gray-200" data-testid="rte-toolbar-expanded">
      ...
      <EditorColorPicker data-testid="rte-highlight-color" :model-value="highlightColor" dropdown-align="ctr" @update:model-value="setHighlightColor($event)">
        <template #trigger="{ color, toggle }">
          <button type="button" class="flex flex-col items-center gap-1 cursor-pointer p-1 hover:bg-gray-100 rounded" @click="toggle">
  ...
  <EditorRichTextEditorRichtextEditorModal
    ...
    :text-color="ref(textColor)"
    :highlight-color="ref(highlightColor)"
    :set-font-color="setFontColor"
    :set-highlight-color="setHighlightColor"
    :set-align="setAlign"
    :is-active-align="isActiveAlign"
    :can-undo="ref(canUndo)"
    :can-redo="ref(canRedo)"
    :undo="undo"
    :redo="redo"
