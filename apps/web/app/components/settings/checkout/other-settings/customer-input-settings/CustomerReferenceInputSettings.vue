<template>
  <div class="py-2 flex flex-col">
    <span class="mb-2">
      {{ getEditorTranslation('description') }}
    </span>
    <div class="flex justify-between mb-2">
      <UiFormLabel class="mb-1">{{ getEditorTranslation('label') }}</UiFormLabel>
      <SfSwitch
        v-model="showCustomerReferenceComponent"
        class="checked:bg-editor-button checked:before:hover:bg-editor-button checked:border-gray-500 checked:hover:border:bg-gray-700 hover:border-gray-700 hover:before:bg-gray-700 checked:hover:bg-gray-300 checked:hover:border-gray-400"
      />
    </div>
  </div>
</template>

<script setup lang="ts">
import { SfSwitch } from '@storefront-ui/vue';

const { updateSetting, getBooleanSetting } = useSiteSettings('showCustomerReferenceComponent');

const showCustomerReferenceComponent = computed({
  get: () => getBooleanSetting(),
  set: (value) => updateSetting(value.toString()),
});
</script>

<i18n lang="json">
{
  "en": {
    "label": "Your Reference Input Field",
    "description": "Show or hide the \"Your reference\" input field during checkout."
  },
  "de": {
    "label": "Your Reference Input Field",
    "description": "Show or hide the \"Your reference\" input field during checkout."
  }
}
</i18n>
