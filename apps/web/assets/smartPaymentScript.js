...
let scriptElement = document.querySelector('#paypal-smart-payment-script');
let clientId = scriptElement.dataset.clientId;
let clientToken = scriptElement.dataset.clientToken;
let currency = scriptElement.dataset.currency;
let appendTrailingSlash = scriptElement.dataset.appendTrailingSlash;
let locale = scriptElement.dataset.locale;
...
window.resolveAfterItemAddedToBasket = function resolveAfterItemAddedToBasket(buttonId) {
  return new Promise((resolve, reject) => {
    try {
      let button = $('#paypal-button-container_' + buttonId);
      if (
        button.parents('.main').length > 0 &&
        document.querySelector('.add-to-basket-container > button') != undefined
      ) {
        if (document.querySelector('.add-to-basket-container > button').classList.contains('disabled')) {
          reject();
        }
        ...
        document.addEventListener(
          ...
          async function (e) {
            if (document.querySelector('#add-item-to-basket-overlay')) {
              const modal = document.querySelector('#add-item-to-basket-overlay > div');
    ...
    paypal_plenty_sdk
      .Buttons({
        ...
        createOrder: function (data, actions) {
          ...
          return resolveAfterItemAddedToBasket(buttonId)
            .then(() => {
              return fetch(
                '/rest/payment/payPal/smart_payment/create/' + foundingSource + (appendTrailingSlash ? '/' : ''),
                {
                  method: 'post',
                },
              )
        ...
        onShippingChange: function (data, actions) {
          ...
          return fetch('/rest/payment/payPal/shipping_options' + (appendTrailingSlash ? '/' : ''), {
            method: 'post',
            body: formData,
          })
            .then(function (res) {
              ...
              if (response.shippingOptions.options) {
                ...
                actions.order
                  ...
                  .catch(function (error) {
                    console.log('error while patching shipping options', error);
      ...
      .render('#paypal-button-container_' + buttonId);
  }
};
...
window.renderSmartPaymentButton = function renderSmartPaymentButton(
  ...
  labelStyle,
  buttonColor,
  buttonColorPayLater = 'gold',
  payLaterActive = false,
) {
  $('.paypal-checkout-button-container').each(function (index, buttonContainer) {
    let checkoutButton = $(buttonContainer).parent().find('div > button.btn.btn-block').css('display', 'none');

    $(buttonContainer).html('').css('display', 'block');
    renderButton(smartMethod, $(buttonContainer).attr('id'), labelStyle, buttonColor);
    if (smartMethod == 'paypal' && payLaterActive) {
      ...
      renderButton('paylater', $(buttonContainer).attr('id'), labelStyle, buttonColorPayLater);
    }
  });
};

window.renderButton = function renderButton(smartMethod, bttId, labelStyle, buttonColor = 'black') {
  let value;
  if (paypal_plenty_sdk.isFundingEligible(smartMethod)) {
    paypal_plenty_sdk
      .Buttons({
        ...
        // Call your server to set up the transaction
        createOrder: function (data, actions) {
          return fetch('/rest/payment/payPal/smart_payment/create_order' + (appendTrailingSlash ? '/' : ''), {
            method: 'post',
          })
            .then(function (res) {
              return res.json();
...
window.renderReinitSmartButton = function renderReinitSmartButton(smartMethod, orderId, labelStyle, buttonColor) {
  renderReinitButton(smartMethod, orderId, labelStyle, buttonColor);
};

window.renderReinitButton = function renderReinitButton(smartMethod, orderId, labelStyle, buttonColor = 'black') {
  let value;
  if (paypal_plenty_sdk.isFundingEligible(smartMethod)) {
    paypal_plenty_sdk
      .Buttons({
        ...
        createOrder: function (data, actions) {
          return fetch(
            '/rest/payment/payPal/smart_payment/' + orderId + '/reinit_create_order' + (appendTrailingSlash ? '/' : ''),
            {
              method: 'post',
            },
          )
...
window.paypalValidateCheckout = function paypalValidateCheckout() {
  return new Promise((resolve, reject) => {
    try {
      var loadingScreen = document.querySelector('#paypal_loading_screen');
      ...
      if (typeof window.ceresStore.state.checkout === 'object') {
        ...
        return fetch('/rest/io/order/additional_information' + (appendTrailingSlash ? '/' : ''), {
          headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': token },
          method: 'POST',
          body: JSON.stringify(params),
        })
          .then(function (res) {
            return fetch('/rest/io/checkout/payment' + (appendTrailingSlash ? '/' : ''), {
...
paypalLoadScript({
  ...
  .then((paypal_plenty_sdk) => {
    ...
    return fetch('/rest/payment/payPal/smart_payment/handle_founding_sources' + (appendTrailingSlash ? '/' : ''), {
    ...
    paypal_plenty_sdk.HostedFields.render({
      ...
      createOrder: function () {
        ...
        createOrderUrl = orderId
          ? '/rest/payment/payPal/smart_payment/' + orderId + '/reinit_create_order' + (appendTrailingSlash ? '/' : '')
          : '/rest/payment/payPal/smart_payment/create_order' + (appendTrailingSlash ? '/' : '');
