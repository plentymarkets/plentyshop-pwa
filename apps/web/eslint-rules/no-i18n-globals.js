...
export const noI18nGlobals = {
  meta: {
    ...
    messages: {
      noGlobalT:
        'Avoid using global $t function. Import the t function from the i18n composable instead (e.g., `const { t } = useI18n();`).',
      noGlobalN:
        'Avoid using global $n function. Import the n function from the i18n composable instead (e.g., `const { n } = useI18n();`).',
      noI18nN:
        'Avoid using {{ name }} i18n currency formatter function. Use the usePriceFormatter composable instead (e.g., `const { format } = usePriceFormatter();`).',
    },
    schema: [],
  },
      ...
      Program() {
        ...
        function checkSourceCode(node) {
          ...
          if (node.type === 'VExpressionContainer' || node.type === 'VDirectiveKey') {
            ...
            if (text.includes('$t(')) {
              context.report({ node, messageId: 'noGlobalT' });
            }

            if (text.includes('$n(')) {
              context.report({ node, messageId: 'noGlobalN' });
            } else if (text.includes('n(') && text.includes(", 'currency'")) {
              context.report({
                node,
                messageId: 'noI18nN',
                data: { name: text.includes('$n(') ? '$n' : 'n' },
              });
            }
          }
