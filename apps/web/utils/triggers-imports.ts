...
const nuxtModuleTriggers = import.meta.glob(
  '/modules/*/runtime/components/**/settings/*/*ToolbarTrigger.vue',
  { import: 'default' }
)  as Record<string, Loader>

const coreTriggers = import.meta.glob(
...
function slug(path: string) {
  ...
  return match
    ? stripOrderPrefix(match[1]).toLowerCase()
    : ''
}
...
export const triggersModules =
  Object.entries(ordered).map(([slug, loader]) => ({
    slug: slug,
