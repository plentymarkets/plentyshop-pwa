...
- The `fallbackLocale` in `i18n.config.ts` has to match the system default language.
- The `defaultLocale` in `nuxt.config.ts` has to match the system default language.
...
Suppose you want to add a new locale, like Spanish. After adding Spanish to the plentysystems system settings, you have to add it to the available `numberFormats` and `locales`. In `locales`, you specify the name of the translation file.
